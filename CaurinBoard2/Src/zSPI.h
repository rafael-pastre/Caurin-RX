...
HAL_StatusTypeDef SPI_read_register(SPI_HandleTypeDef spi_bus,
		...
		uint8_t* pvalue, uint32_t timeout);	// received data is stored in value
HAL_StatusTypeDef SPI_burst_read(SPI_HandleTypeDef spi_bus,
		GPIO_TypeDef* ss_gpio_port, uint16_t ss_pin, uint8_t start_addr,
		uint8_t* pvalue, uint16_t size, uint32_t timeout);
HAL_StatusTypeDef SPI_write_register(SPI_HandleTypeDef spi_bus,
		GPIO_TypeDef* ss_gpio_port, uint16_t ss_pin, uint8_t reg_addr,
		uint8_t value, uint32_t timeout);
HAL_StatusTypeDef SPI_burst_write(SPI_HandleTypeDef spi_bus,
		GPIO_TypeDef* ss_gpio_port, uint16_t ss_pin, uint8_t start_addr,
		uint8_t* value, uint16_t size, uint32_t timeout);
